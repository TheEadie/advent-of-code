namespace AdventOfCode2021.Day23
{
    public class Day23
    {
        ...
        private static int GetSideRoomSize(GameState currentState, char room)
        {
            return currentState.GetSideRoom(room).Length;
        }
        ...
        private int GetSideRoomEntrance(char room)
        {
            return (room - 'A') * 2 + 2;
        }
        ...
        private int GetNumberOfAmphipodsInSideRoom(char room, GameState state)
        {
            return state.GetSideRoom(room).Count(x => x != '.');
        }
        ...
        private record GameState(string Hallway, string A, string B, string C, string D)
        {
            ...
            public bool AreAllPresentHome(char room)
            {
                return GetSideRoom(room).All(x => x == '.' || x == room);
            }
