...
public class Day17
{
    ...
    private static int GetMaxYVelocity(int bottomY) => -bottomY - 1;

    private static int GetMinYVelocity(int bottomY) => bottomY;

    private int GetMinXVelocity(int lower, int upper) => Enumerable.Range(0, 100).Where(x => lower <= GetLimitOfX(x) && GetLimitOfX(x) <= upper).Min() - 1;

    private int GetLimitOfX(int vx) => vx / 2 * (1 + vx);
